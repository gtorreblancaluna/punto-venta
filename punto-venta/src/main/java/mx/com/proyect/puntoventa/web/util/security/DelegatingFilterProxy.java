...
import mx.com.proyect.puntoventa.web.service.AccountService;
	...
	public class DelegatingFilterProxy extends GenericFilterBean {
		...
		private Log log = LogFactory.getLog( DelegatingFilterProxy.class );
		@Autowired
		...
		protected void invokeDelegate(
				Filter delegate, ServletRequest req, ServletResponse resp, FilterChain filterChain)
				throws ServletException, IOException {
			...
			if (!isAjaxRequest(request)) {
				/**/
//				String urlDwr = ((HttpServletRequest)request).getRequestURL().toString();
//				if( urlDwr.contains("/dwr/") && urlDwr.endsWith(".js") ){
//				    HttpServletResponse respw = new HttpServletResponseWrapper(response) {
...
//
//					delegate.doFilter(request, respw, filterChain);
//					return;
//				}
				/**/
			...
			delegate.doFilter(request, redirectResponseWrapper, filterChain);
//			filterChain.doFilter(request, redirectResponseWrapper);
			...
			if (redirectResponseWrapper.getRedirect() != null) {
				...
				String agent_redirect_url = request.getParameter("redirect_url");

				log.debug("agent_id: " + agent_id);
				log.debug("agent_redirect_url: " + agent_redirect_url);
				...
				if (redirectURL.indexOf("login_error=1") == -1) {
					...
					if (agent_id != null) {
						...
						String agent_tk = request.getParameter("agent_tk");
						log.debug("agent_tk: " + agent_tk);
						int agent_tk_int = Integer.parseInt(agent_tk);

						log.debug("la peticion de login fue realizada por un agente "
								+ agent_id);
...
//						log.debug("saldo: " + account.getBalanceDecoded());
//						double saldo = Double.parseDouble(account
//								.getBalanceDecoded());
//						String balanceAmount = myFormatter.format(saldo);
//
//						// Encripta los datos de regreso al agente
//						byte[] encryptData = AgentEncryptDecryptUtil
//								.encryptReturnData(
//										agent_tk_int,
//										username,
//										balanceAmount,
...
//										account.getLastName(),
//										imgContextPath
//												+ ApplicationConstants.AGENT_SECURITY_FILE_PATH
//												+ "agent_info");

						// Arma la cadena de regreso en data
//						log.debug("name: " + account.getName());
//						log.debug("lastName: " + account.getLastName());
//						String url = agent_redirect_url
//								+ "?data="
//								+ (new String(HexCodec.bytesToHex(encryptData))
//										.toUpperCase());
...
//									AbstractProcessingFilter.ACEGI_SECURITY_LAST_EXCEPTION_KEY));
					content = "error:" + authenticationException.getMessage();
